...
====================================
This guide will show you how to build boldd (headless client) for OSX.
...
5.  (Optional) You can also install boldd to your path:
...
4. Enter "bold-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `boldd` for your own use.

boldd/bold-cli binaries are not included in the bold-Qt.app bundle.

If you are building `boldd` or `bold-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the Bold-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./boldd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./boldd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=boldrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/Bold/bold.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/Bold/bold.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/Bold/debug.log

Other commands:
-------

    ./boldd -daemon # to start the bold daemon.
    ./bold-cli --help  # for a list of command-line options.
    ./bold-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure Bold with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the Bold wallet:
...
You have the choice to build the GUI Bold wallet as a Mac OSX app, described in “How to build the Bold-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the Bold-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find Bold-Qt.dmg inside your Bold folder. Open and install the wallet like any typical Mac app.
