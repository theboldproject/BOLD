...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->Unlock(strWalletPass, anonymizeOnly))
        throw JSONRPCError(RPC_WALLET_PASSMUEASE_INCORRECT, "Error: The wallet passphrase entered was incorrect.");
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zMUE is currently disabled due to maintenance.");
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "Overview: Convert zMUE (zerocoins) into MUE. \n"
            "amount: amount to spend\n"
            "mintchange: if there is left over MUE (change), the wallet can convert it automatically back to zerocoins [true]\n"
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zMUE
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CzMUETracker* zphrTracker = pwalletMain->zphrTracker.get();
...
UniValue setzphrseed(const UniValue& params, bool fHelp)
{
    ...
    CzMUEWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzphrseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "getzphrseed\n"
            "\nCheck archived zMUE list to see if any mints were added to the blockchain.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic zMUE seed.\n"
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zMUE seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zMUE to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zMUE to generate.\n"
    ...
    CzMUEWallet* zwallet = pwalletMain->zwalletMain;
...
UniValue dzphrstate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
                "dzphrstate\n"
                        "\nThe current state of the mintpool of the deterministic zMUE wallet.\n" +
                HelpRequiringPassphrase() + "\n"
...
void static SearchThread(CzMUEWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZMUE(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzphr(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 3)
        throw runtime_error(
            "searchdzphr\n"
            "\nMake an extended search for deterministically generated zMUE that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential zMUE to start with.\n"
            "2. \"range\"       (numeric) How many zMUE to generate.\n"
