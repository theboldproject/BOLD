...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BOLD);
    ...
    if (!settings.contains("nAnonymizeBoldAmount"))
        settings.setValue("nAnonymizeBoldAmount", 1000);

    nAnonymizeBoldAmount = settings.value("nAnonymizeBoldAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBoldAmount"))
        SoftSetArg("-anonymizeboldamount", settings.value("nAnonymizeBoldAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bold.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBoldAmount:
            return QVariant(nAnonymizeBoldAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBoldAmount:
            nAnonymizeBoldAmount = value.toInt();
            settings.setValue("nAnonymizeBoldAmount", nAnonymizeBoldAmount);
            emit anonymizeBoldAmountChanged(nAnonymizeBoldAmount);
            break;
        case CoinControlFeatures:
