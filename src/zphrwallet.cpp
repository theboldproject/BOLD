...
CzMUEWallet::CzMUEWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMUESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMUESeed_deprecated()) {
                    LogPrintf("%s: Updated zMUE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzMUEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMUECount(nCountLastUsed);
    else if (!walletdb.ReadZMUECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMUEWallet::Lock()
{
    seedMaster = 0;
}

void CzMUEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMUEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMUE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMUEWallet::LoadMintPoolFromDB()
{
...
void CzMUEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMUEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMUEWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzMUEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMUE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzMUEWallet::SeedToZMUE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMUEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMUEWallet::GenerateDeterministicZMUE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMUEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMUEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
