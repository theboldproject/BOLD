...
#ifndef MONETARYUNIT_ZMUEWALLET_H
#define MONETARYUNIT_ZMUEWALLET_H
...
public:
    CzMUEWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMUE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMUE(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MONETARYUNIT_ZMUEWALLET_H
