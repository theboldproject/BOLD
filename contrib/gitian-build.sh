...
VERSION=
commit=false
url=https://github.com/boldproject/bold
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the bold, gitian-builder, gitian.sigs, and bold-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/boldproject/bold
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit bold=${COMMIT} --url bold=${url} ../bold/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../bold/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/bold-*.tar.gz build/out/src/bold-*.tar.gz ../bold-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit bold=${COMMIT} --url bold=${url} ../bold/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/bold-*.zip build/out/bold-*.exe ../bold-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit bold=${COMMIT} --url bold=${url} ../bold/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/bold-*.tar.gz build/out/bold-*.dmg ../bold-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit bold=${COMMIT} --url bold=${url} ../bold/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../bold/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/bold-*.tar.gz build/out/src/bold-*.tar.gz ../bold-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../bold/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/bold-*win64-setup.exe ../bold-binaries/${VERSION}
	    mv build/out/bold-*win32-setup.exe ../bold-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../bold/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/bold-osx-signed.dmg ../bold-binaries/${VERSION}/bold-${VERSION}-osx.dmg
	fi
	popd || exit
